...
@section('content')
    <div class="row">
        <div class="col-md-8">
            ...
            <!-- Evidence Photos Section - Visible to all users including checkers -->
            @php
                // Get the escrow transaction related to this dispute
                $escrow = $dispute->escrowTransaction;
                // Get photos if available - These are critical for all roles (makers, checkers, buyers, sellers)
        ...
        <div class="col-md-4">
            <div class="card mb-4">
                ...
                <div class="card-body">
                        ...
                        <ul class="mb-0 mt-2">
                            @if($dispute->status == 'Pending')
                                ...
                                @elseif(auth()->user()->role === 'checker')
                                    <li>Wait for a maker to resolve this dispute</li>
                                    <li>Once resolved, you'll be able to approve or reject the resolution</li>
                                @endif
                            @else
                    ...
                    @php
                        // Check if there are any pending actions related to this dispute
                    ...
                    @if($dispute->status == 'Pending')
                        <!-- Only show the appropriate actions based on user role -->
                        @if(auth()->user()->role === 'maker')
                            <!-- For makers, always show the Resolve Dispute button -->
                        ...
                        @elseif(auth()->user()->role === 'checker')
                            <!-- For checkers, only show actions if there are pending resolutions -->
                                ...
                                <div class="alert alert-primary">
                                    ...
                                    <ul class="mb-0 mt-2">
                                        @foreach($pendingActions as $action)
                                            <li>
                                                <a href="{{ route('admin.maker-checker.show', $action->action_id) }}" class="alert-link">
                                                    Review resolution by {{ $action->maker->name }}
                                                </a>
                                            </li>
                                        @endforeach
                                    </ul>
                            ...
                            @else
                                <!-- If no pending actions yet -->
                                <div class="alert alert-secondary">
                                    <i class="fas fa-info-circle me-2"></i>
                                    This dispute is awaiting initial resolution by a maker. As a checker, you will be able to review and approve/reject the resolution once it has been submitted by a maker.
                                </div>
                            @endif
                        @endif
                    @endif
